...
static void do_test() {
    ...
    IOSurfaceRef surface = IOSurfaceLookup(1);
    printf("<=2\n");
...
int main() {
    ...
    while(reg = IOIteratorNext(it)) {
        io_name_t name;
        assert(!IORegistryEntryGetNameInPlane(reg, kIOServicePlane, name));
        if(!strcmp(name, "vram")) {
    ...
    printf("vram phys: %08x\n", vram_phys);
    printf("vram size: %08x\n", vram_length);
    ...
    if(!vram_virt) {
        printf("Error: Couldn't find a virtual address for vram.\n");
        return 1;
    }
    
    printf("** Please turn off this device's screen and press enter.\n");
    
    do_test();
    

    printf("** Okay, now turn on this device's screen and press enter.\n");

    do_test();

    printf("** Thanks!  Please post the output on the pad.\n");
